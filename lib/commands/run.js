...
function readPayload(script, callback) {
  async.map(
    ...
    function readPayloadFile(payloadSpec, next) {
      ...
      csv(data, csvOpts, function(err, parsedData) {
        ...
        if (_.isArray(parsedData)) {
          payloadSpec.values = _.zipObject(payloadSpec.fields, parsedData[0]);
        } else {
          payloadSpec.values = _.zipObject(payloadSpec.fields, parsedData);
        }
    ...
    function done(err, results) {
      ...
      // Adds the payload data to the initial context variables,
      // then overlays the inline variables back on top.
      let inlineVariables = script.variables;
      script.variables = _.assign(
